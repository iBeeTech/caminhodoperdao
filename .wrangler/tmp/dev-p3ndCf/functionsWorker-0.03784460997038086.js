...
// .wrangler/tmp/pages-KGthX2/functionsWorker-0.03784460997038086.mjs
...
async function getByEmail(DB, email) {
  const stmt = DB.prepare(
    "SELECT id, email, name, status, payment_provider, payment_ref, created_at, paid_at FROM registrations WHERE email = ?"
...
async function getByPaymentRef(DB, paymentRef) {
  const stmt = DB.prepare(
    "SELECT id, email, name, status, payment_provider, payment_ref, created_at, paid_at FROM registrations WHERE payment_ref = ?"
  ).bind(paymentRef);
  return await stmt.first() ?? null;
}
...
async function insertRegistration(DB, input) {
  const stmt = DB.prepare(
    "INSERT INTO registrations (id, email, name, status, payment_provider, payment_ref, created_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, datetime('now'))"
  ).bind(
    input.id,
...
var MockPixProvider = class {
  ...
  async createCharge({ name, email }) {
    ...
    return { payment_ref: ref, qrCodeText };
  }
};
...
async function handleRegister(env, body) {
  ...
  if (!name || !email) {
    return badRequest("name_and_email_required");
  }
...
// .wrangler/tmp/bundle-yo72ln/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-yo72ln/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.03784460997038086.js.map
