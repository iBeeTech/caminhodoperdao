...
// .wrangler/tmp/pages-3Ijupf/functionsWorker-0.880342889319996.mjs
...
__name(getByEmail, "getByEmail");
__name2(getByEmail, "getByEmail");
async function getByPaymentRef(DB, paymentRef) {
  const stmt = DB.prepare(
...
__name(getByPaymentRef, "getByPaymentRef");
__name2(getByPaymentRef, "getByPaymentRef");
async function insertRegistration(DB, input) {
  const stmt = DB.prepare(
...
__name(insertRegistration, "insertRegistration");
__name2(insertRegistration, "insertRegistration");
async function updateRegistration(DB, email, input) {
  const stmt = DB.prepare(
...
__name2(updateRegistration, "updateRegistration");
async function updatePaymentRef(DB, email, provider, paymentRef) {
  const stmt = DB.prepare(
    "UPDATE registrations SET payment_ref = ?, payment_provider = ? WHERE email = ?"
  ).bind(paymentRef, provider, email.toLowerCase());
  await stmt.run();
}
__name(updatePaymentRef, "updatePaymentRef");
__name2(updatePaymentRef, "updatePaymentRef");
async function markAsPaid(DB, email, paymentRef) {
  const stmt = DB.prepare(
    "UPDATE registrations SET status = 'PAID', paid_at = datetime('now') WHERE email = ? AND payment_ref = ?"
  ).bind(email.toLowerCase(), paymentRef);
  await stmt.run();
}
__name(markAsPaid, "markAsPaid");
__name2(markAsPaid, "markAsPaid");
...
__name(expirePending, "expirePending");
__name2(expirePending, "expirePending");
...
__name2(countActive, "countActive");
async function countActiveSleep(DB) {
  const row = await DB.prepare(
    "SELECT COUNT(1) as total FROM registrations WHERE status IN ('PENDING','PAID') AND sleep_at_monastery = 1"
  ).first();
  return row?.total ?? 0;
}
__name(countActiveSleep, "countActiveSleep");
__name2(countActiveSleep, "countActiveSleep");
...
__name2(validateSignature, "validateSignature");
async function handleWebhook(env, request, body) {
...
var MockPixProvider = class {
  static {
    __name(this, "MockPixProvider");
  }
  static {
    __name2(this, "MockPixProvider");
  }
  async createCharge({ name, email }) {
...
__name(getPaymentProvider, "getPaymentProvider");
__name2(getPaymentProvider, "getPaymentProvider");
var MAX_TOTAL = 400;
var MAX_SLEEP = 100;
...
__name2(handleAvailability, "handleAvailability");
...
async function handleReissue(env, body) {
  if (!body || typeof body !== "object") {
    return badRequest("invalid_body");
  }
...
__name(handleReissue, "handleReissue");
__name2(handleReissue, "handleReissue");
...
async function handleStatus(env, email) {
  if (!email) return badRequest("email_required");
  if (!isValidEmail(email)) return badRequest("invalid_email");
  await expirePending(env.DB);
  const registration = await getByEmail(env.DB, email);
  ...
  const message = registration.status === "PAID" ? "Inscri\xE7\xE3o confirmada" : null;
  return json(200, {
    exists: true,
...
// .wrangler/tmp/bundle-XPfg9C/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-XPfg9C/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.880342889319996.js.map
